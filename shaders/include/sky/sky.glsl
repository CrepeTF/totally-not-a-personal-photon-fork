...
//#endif
	...
	// Rain
	vec3 rain_sky = get_weather_color() * (0.3 - exp2(-0.8 / clamp01(ray_dir.y)));
...
const vec3 end_sun_color = vec3(1.0, 0.3, 1.0);
...
vec3 draw_end_sun(vec3 ray_dir) {
    ...
    float flare = texture(noisetex, vec2(theta, r - 0.5 * frameTimeCounter)).x;
          flare = pow5(flare) * exp(-1.0 * (r - sun_angular_radius)); // Increased exponent from -2.0 to -3.0
          flare = r < sun_angular_radius ? 0.0 : flare;

    return end_sun_color * rcp(sun_solid_angle) * max0(sun_disk + 0.03 * flare); // Reduced flare intensity from 0.1 to 0.05
}
