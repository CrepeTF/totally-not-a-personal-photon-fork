...
vec3 get_rain_color() {
	return mix(0.063, 0.05, smoothstep(-0.1, 0.5, sun_dir.y)) * 0.35 * sunlight_color * vec3(0.8, 0.8, 0.8);
}

vec3 get_snow_color() {
	return mix(0.060, 0.05, smoothstep(-0.1, 0.5, sun_dir.y)) * sunlight_color * vec3(0.49, 0.65, 1.00);
}

vec3 get_sandstorm_color() {
	return mix(0.063, 0.05, smoothstep(-0.1, 0.5, sun_dir.y)) * sunlight_color * vec3(1.00, 0.83, 0.60);
}
